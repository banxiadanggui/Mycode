...
def solve(area: dict, area_dict: dict[list], crop: dict, crop_dict: dict[list], cost: dict[dict[list]], promised: dict, price: dict):
    ...
    model.obj = Objective(expr=sum(model.A[i,g-1,area] * (cost[key]['price'][cost[key]['crop'].index(g)] * cost[key]['per'][cost[key]['crop'].index(g)] - cost[key]['cost'][cost[key]['crop'].index(g)])  for key in area_dict.keys() for g in cost[key]['crop'] for area in area_dict[key] for i in range(2, 16)), sense=maximize)
